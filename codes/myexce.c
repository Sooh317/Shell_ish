...
void myexec(job* jb, char *envp[]){	
	...
	if(!strcmp(jb->process_list->program_name, "fg")){
	...
	for(i = 0; i < process_num; i++){
		if(setpgid(pid[i], pid[0]) < 0) perror("set pgid error"), exit(1);
		if(jb->mode == FOREGROUND){
			if(tcsetpgrp(0, pid[0]) < 0) perror("tcsetpgrp error"), exit(1);
			if(waitpid(pid[i], &status[i], WUNTRACED) < 0) perror("wait error"), exit(1);
			if(tcsetpgrp(0, getpgrp()) < 0) perror("tcsetpgid error"), exit(1);
			if(WIFSTOPPED(status[i])) make_sus(*cur_process, pid[i], pid[0]);
		}
